...
apa.cor.table(analytic.data, filename="regresstion_table1.doc", table.number = 1)
...
interaction.regression <- lm(exam~x.centered+z.centered+I(x.centered*z.centered), 
                             data=analytic.data, na.action = na.exclude) #na.action part means drop row with missing values

#results
apa.reg.table(interaction.regression, filename="Table2.doc", table.number = 2) # get TABLE 
summary(interaction.regression) #get values for TEXT 
...
# GENERATING CROSS-SECTION LINES +/- 1 SD -------------------------------------------------------------------------------------------
...
apa.reg.table(simple.slope.plus.1SD) # get table 
summary(simple.slope.plus.1SD) # get values for text 
...
apa.reg.table(simple.slope.minus.1SD) # get table 
summary(simple.slope.minus.1SD) # get values for text 
...
#enter values for 3D plot by referring to summary results 
...
intr.plot(b.0= 49.2104,b.x=15.2537,b.z=9.5471,b.xz=6.0481,
          ...
          expand=1,hor.angle=60,gray.scale=TRUE, line.wd=4,zlim=c(0,100)) #save manually 
#-------------------------------------------------------------------------------------------------
